...
// dropReadHandleAll invalidates any cached read-only fd for the given NFS handle
// across every live connection, WITHOUT touching cached write handles. Called
// after a successful WRITE so a read-only fd opened before the write is not
// reused by a later READ on snapshot/buffered backends (which would return
// pre-write bytes); the next READ reopens. With nconnect the read may have been
// cached on a different connection than the WRITE, so this broadcasts. It is a
// cheap no-op (a per-connection map lookup) when no read fd is cached, keeping
// the write hot path fast for write-only workloads. Returns the first close
// error encountered (nil if none).
func (s *Server) dropReadHandleAll(handle []byte) error {
	s.connsMu.Lock()
	conns := make([]*conn, 0, len(s.conns))
	for c := range s.conns {
		conns = append(conns, c)
	...
	var firstErr error
	for _, c := range conns {
		if err := c.dropReadHandle(handle); err != nil && firstErr == nil {
			firstErr = err
		}
	}
