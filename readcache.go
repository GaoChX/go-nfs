...
type readCache struct {
	...
	// gens tracks a per-key invalidation generation for cross-connection drops
	// (SETATTR/REMOVE/RENAME/CREATE). readHandleFor samples a key's generation
	// before Open and publishes via putIfFresh only if that key's generation has
	// not moved, so a read-only fd opened under the old mode/size cannot be cached
	// after an intervening invalidation — which would otherwise serve
	// pre-chmod/pre-truncate data. Keyed per handle (not a cache-wide counter) so
	// invalidating one file cannot refuse a concurrent open of an unrelated file.
	// See keyGens. Guarded by mu.
	gens *keyGens
...
// the in-flight-open count, before Open so putIfFresh can detect an invalidation
// that ran during the open window. Must be paired with exactly one
// resolveGen(key) on every code path after the open.
func (c *readCache) sampleGen(key string) uint64 {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// resolveGen ends the open started by sampleGen(key). Once no opens remain
// outstanding for key, the generation entry is dropped (the memory bound).
func (c *readCache) resolveGen(key string) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
// SETATTR/REMOVE/RENAME/CREATE dropped the handle. This does NOT resolve the
// open — the caller still calls resolveGen(key) exactly once.
...
// invalidate detaches and returns the handle for key (like remove) and bumps
// key's invalidation generation so a read-only fd opened under now-stale
// attributes before this call cannot be published afterward (putIfFresh). Used
// by cross-connection drops (SETATTR/REMOVE/RENAME/CREATE). The per-key gen is
// bumped only if an open for this key is in flight (keyGens.invalidate is a
// no-op otherwise).
