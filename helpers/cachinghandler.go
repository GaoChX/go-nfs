...
func (c *CachingHandler) searchReverseCache(f billy.Filesystem, path string) []byte {
	...
	for _, id := range c.reverseHandles[path] {
		if candidate, ok := c.activeHandles.Get(id); ok {
			if sameFilesystem(candidate.f, f) {
				return id[:]
			}
		}
	}
...
// sameFilesystem reports whether two billy.Filesystem values are the same
// instance. It cannot use a bare `a == b`: a valid custom billy.Filesystem whose
// concrete type is non-comparable (e.g. a struct containing a map or slice)
// would panic on interface equality. The common real implementations (osfs,
// chroot, memfs) are pointer types, so compare pointers when both dynamic types
// are pointer-kinded (identity, and cheap). Otherwise fall back to
// reflect.DeepEqual, which is what this code used before the pointer-compare
// optimization and does not panic. This restores panic-safety for custom
// handlers while keeping the fast path for the usual pointer-typed filesystems.
func sameFilesystem(a, b billy.Filesystem) bool {
	if a == nil || b == nil {
		return a == nil && b == nil
	}
