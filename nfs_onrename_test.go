...
// pathRenameHandler is a stateful, path-based handler that does NOT implement
// CachedHandleLookup, so handleForInvalidation falls back to ToHandle gated on
// path existence (fs.Stat). It derives a stable handle per path (like a handler
// keying on device+inode) so the same path always yields the same handle.
type pathRenameHandler struct {
	fs          billy.Filesystem
	dirHandle   []byte
	invalidated [][]byte
}

func (h *pathRenameHandler) Mount(context.Context, net.Conn, MountRequest) (MountStatus, billy.Filesystem, []AuthFlavor) {
	return MountStatusOk, h.fs, nil
}
func (h *pathRenameHandler) Change(_ context.Context, fs billy.Filesystem) billy.Change {
	if c, ok := fs.(billy.Change); ok {
		return c
	}
	return nil
}
func (h *pathRenameHandler) FSStat(context.Context, billy.Filesystem, *FSStat) error { return nil }
func (h *pathRenameHandler) ToHandle(_ context.Context, _ billy.Filesystem, path []string) []byte {
	return []byte("path-handle-" + h.fs.Join(path...))
}
func (h *pathRenameHandler) FromHandle(_ context.Context, fh []byte) (billy.Filesystem, []string, error) {
	if bytes.Equal(fh, h.dirHandle) {
		return h.fs, []string{}, nil
	}
	return nil, nil, errInvalidRenameHandle
}
func (h *pathRenameHandler) InvalidateHandle(_ context.Context, _ billy.Filesystem, handle []byte) error {
	h.invalidated = append(h.invalidated, handle)
	return nil
}
func (h *pathRenameHandler) HandleLimit() int { return 1024 }

// TestOnRenameInvalidatesPreRenameSourceHandle verifies that for a handler
// without CachedHandleLookup (the ToHandle-fallback path), RENAME invalidates
// the source path's handle even though after the rename the source path no
// longer exists (so a post-rename re-resolve alone would miss it). The
// pre-rename handle is captured and unioned into the invalidation set.
func TestOnRenameInvalidatesPreRenameSourceHandle(t *testing.T) {
	dir := t.TempDir()
	fs := osfs.New(dir)
	if f, err := fs.Create("src.dat"); err != nil {
		t.Fatalf("create: %v", err)
	...
	dirHandle := []byte("rename-dir-handl")
	h := &pathRenameHandler{fs: fs, dirHandle: dirHandle}
	srv := &Server{Handler: h}
	c := &conn{Server: srv}
	srv.registerConn(c)
	defer c.drainCaches()
	...
	// The source path's pre-rename handle must have been invalidated, even though
	// the source path no longer exists post-rename.
	srcHandle := []byte("path-handle-" + fs.Join("src.dat"))
	found := false
	for _, inv := range h.invalidated {
		if bytes.Equal(inv, srcHandle) {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("source handle %q not invalidated; got %v", srcHandle, h.invalidated)
	}
}
